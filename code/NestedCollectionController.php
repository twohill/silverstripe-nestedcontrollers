...
class NestedCollectionController extends Controller {
	 ...
	 */ 
	protected static $recordController = 'NestedModelController';
	protected static $recordType = 'DataObject';
	 ...
	 */
	public function __construct(Controller $parentController, SS_HTTPRequest $request) {
	...
	public function getAllRecords() {
		...
		$SQL_end = (int) $this->stat('results_per_page');
		if (isset($_GET['letter'])) {
			$SQL_where = $this->stat('sort_on') . " LIKE '" . Convert::raw2sql($_GET['letter'][0]) . "%'";
		} else {
			$SQL_where = null;
		}
		$all = DataObject::get($this->stat('recordType'), $SQL_where, null, null, "$SQL_start, $SQL_end");
		/*
		 * TODO: implement permission checking with pagination
		 *
		  if ($all) {
			$allowed = new DataObjectSet();
			foreach ($all as $one) {
				if ($one->canView()) {
					$allowed->push($one);
				}
	...
	function getViewer($action) {
		if($this->parentController) {
			...
			$layoutGenericTemplate = SSViewer::getTemplateFileByType($this->stat('recordType'), 'Layout');
			...
			$layoutActionTemplate = SSViewer::getTemplateFileByType($this->stat('recordType') . "_$action", 'Layout');
